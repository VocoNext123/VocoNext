...
class Generator(torch.nn.Module):
    def __init__(self, h):
        ...
        self.convnext = nn.ModuleList(
            [ConvNeXtBlock(dim=self.dim, intermediate_dim=self.intermediate_dim) for _ in range(self.num_layers)]
        )
    ...
    def forward(self, mel):
        ...
        logamp = self.norm2(logamp.transpose(1, 2)).transpose(1, 2)
        for conv_block in self.convnext2:
            logamp = conv_block(logamp)
        logamp = self.final_layer_norm2(logamp.transpose(1, 2)).transpose(1, 2)
        logamp = self.ASP_output_conv(logamp)

        # PSP Path
        pha = self.PSP_input_conv(mel)
        pha = self.norm(pha.transpose(1, 2)).transpose(1, 2)
        for conv_block in self.convnext:
            pha = conv_block(pha)
        pha = self.final_layer_norm(pha.transpose(1, 2)).transpose(1, 2)
        R = self.PSP_output_R_conv(pha)
        I = self.PSP_output_I_conv(pha)

        # Combine log-amplitude and phase into a spectrogram
        pha = torch.atan2(I, R)  # Phase
        rea = torch.exp(logamp) * torch.cos(pha)  # Real part
        imag = torch.exp(logamp) * torch.sin(pha)  # Imaginary part

        # Create complex-valued tensor
        spec = torch.complex(rea, imag)  # Combine real and imaginary parts
